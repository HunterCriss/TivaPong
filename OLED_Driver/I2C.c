...
int main(void)
{
    ...
    while(1){
			OLED_Clear();
			OLED_YX( 0, 0 );
      OLED_Write_String( "Do You Work?" );
			Delay_ms(1000);
			
			OLED_Clear();
			OLED_Draw_Rectangle(10, 10, 20, 30);
			Delay_ms(1000);
		}
}
...
//OLED
void OLED_Command( uint8_t temp){
	  I2C3_Wr(slaveaddress,0x00,temp);
}
...
void OLED_Data( uint8_t temp){
	  I2C3_Wr(slaveaddress,0x40,temp);
}
 ...
 * Description: Sets the cursor X and Y position 
 ******************************************************************************/
...
void OLED_PutChar(char ch ){
   ...
   const uint8_t *base = &OledFont[ch - 32][0];
    ...
    memmove( bytes + 1, base, 8 );
		...
		I2C3_Write_Multiple(slaveaddress,0x40,9,bytes);
}
...
void OLED_Draw_Rectangle(uint8_t x, uint8_t y, uint8_t width, uint8_t height) {
    ...
    // Calculate the end coordinates to ensure the rectangle stays within bounds
    ...
    for (uint8_t i = y; i <= endY; i++) {
        OLED_YX(i, x);
        for (uint8_t j = x; j <= endX; j++) {
            OLED_Data(0xFF); // Set pixel to ON
        }
    }
}